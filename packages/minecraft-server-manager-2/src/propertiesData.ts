...
const propertiesData = {
    ...
    'broadcast-console-to-ops': _p(
        true,
        'true',
        'コンソールコマンドの出力をオンラインのオペレータに送信する',
    ),
    'broadcast-rcon-to-ops': _p(
        true,
        'true',
    ...
    difficulty: _p(
        false,
        'easy',
        'サーバーの難易度（Mob から受けるダメージや、プレイヤーの空腹度、毒のダメージなどに影響する）を決定する',
    ),
    ...
    'entity-broadcast-range-percentage': _p(
        false,
        '100',
        '	エンティティがどの程度近くにいる場合にクライアントに送信するかを制御する',
    ),
    ...
    hardcore: _p(
        true,
        'false',
        'true に設定した場合、プレイヤーが死んだ際にスペクテイターモードになる',
    ),
    ...
    'online-mode': _p(
        true,
        'true',
        'サーバーは、プレイヤーをMinecraftアカウントデータベースに対して接続チェックする',
    ),
    'op-permission-level': _p(
        false,
        '4',
        '/op を使用したときに与えられるデフォルトの権限レベルを設定する',
    ),
    'player-idle-timeout': _p(
        false,
        '0',
        '無操作のプレイヤーがサーバーからキックされるまでの時間を分で設定する',
    ),
    'prevent-proxy-connections': _p(
        true,
        'false',
        'サーバーから送信されたISP/ASがMojangの認証サーバーと異なる場合、プレイヤーはキックされる',
    ),
    ...
    'rate-limit': _p(
        false,
        '0',
        'プレイヤーがキックされるまでに送信できるパケットの最大量を設定する',
    ),
    ...
    'use-native-transport': _p(
        true,
        'true',
        'Linuxサーバーのパフォーマンスの向上：Linuxでのパケット送受信の最適化',
    ),
    'view-distance': _p(
        false,
        '10',
